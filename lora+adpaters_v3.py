...
# LoRA + chained bottleneck adapters + noise curriculum (joint/decoder frozen; no LSTM)
...
PHASE_LORA_LAST_EPOCH = int(os.environ.get("PHASE_LORA_LAST_EPOCH", "2"))
...
class PhasedTrainabilityCallback(Callback):
    def __init__(self, adapter_short, last_lora, last_adapter):
        super().__init__()
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        for n, p in pl_module.named_parameters():
            ...
            elif "joint" in nl or "decoder" in nl or "prediction" in nl:
                p.requires_grad = False
            elif self.adapter_short in nl or "asr_children" in nl:
                p.requires_grad = adapter_on
        if getattr(trainer, "global_rank", 0) == 0:
            print(f"[PhasedTrainability] human_epoch={ep+1}  LoRA={lora_on}  adapter={adapter_on}  joint/decoder=frozen", flush=True)
...
def _collect_trainable_groups(model, adapter_short):
    ...
    grouped = {id(x) for xs in (lora_p, adapter_p) for x in xs}
    for n, p in model.named_parameters():
        if not p.requires_grad or id(p) in grouped:
            continue
        print(f"WARNING: unclassified trainable param -> adapter group: {n}")
        add_once(adapter_p, p)
    return lora_p, adapter_p

def _build_configure_optimizers(model, lora_p, adapter_p, warmup_ratio, num_epochs, steps_per_epoch):
    groups = []
...
for attr in ("joint", "decoder"):
    ...
    if mod is not None:
        for p in mod.parameters():
            p.requires_grad = False
        print(f"FROZEN {attr}: {sum(p.numel() for p in mod.parameters()):,} params")

for m in (model.decoder, model.joint, model.waveform_augmentor, model.spec_augmentation):
    if m is not None:
        m.train()

lora_p, adapter_p = _collect_trainable_groups(model, adapter_short)
co_fn, opt_groups = _build_configure_optimizers(
    model, lora_p, adapter_p,
...
print("=" * 60)
print("STARTING TRAINING (vNext adapters + bottleneck chain; joint/decoder frozen)")
print("=" * 60)
trainer.fit(model)
